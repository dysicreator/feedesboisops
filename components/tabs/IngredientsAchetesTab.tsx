...
const IngredientsAchetesTab: React.FC<IngredientsAchetesTabProps> = ({ currentUser }) => {
  ...
  const ingredientAcheteColumns: ColumnDefinition<IngredientAchete>[] = useMemo(() => [
    { 
      accessor: 'nom', 
      Header: 'Nom Ingrédient',
      cell: (item, data) => {
        const currentAllData = data || allData;
        if (item.nom) return item.nom;
        const ref = currentAllData.parametresData.find(p => p.type === 'ingredientGeneriqueRef' && p.id === item.ingredientRefId);
        return ref?.nom || '—';
      },
  ...
  ], [allData, ingredients, seuilsIngredientsGeneriquesData]);
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    try {
      ...
      console.error("Error deleting ingredient:", { code: error.code, message: error.message });
  ...
  const handleFormSubmit = async (data: Partial<IngredientAchete>) => {
    ...
    if (payload.ingredientRefId && !payload.nom) {
      const ref = allData.parametresData.find(p => p.id === payload.ingredientRefId);
      if (ref) {
          payload.nom = ref.nom;
      }
    }
    ...
    try {
      ...
      console.error("Error saving ingredient:", { code: error.code, message: error.message });
