...
const LotsFabricationTab: React.FC<LotsFabricationTabProps> = ({ currentUser }) => {
  ...
  const handleAddItem = useCallback(() => {
    ...
    setEditingLot({
        ...
        recetteId: '',
        nomProduitFini: '',
        lotNumeroProduitFini: `LOTF-${dateSuffix}-` + Math.random().toString(36).substr(2, 4).toUpperCase(),
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    if (!window.confirm('Êtes-vous sûr de vouloir supprimer ce lot de fabrication ? Le stock des matières premières et conditionnements sera recrédité si applicable.')) {
        return;
    }
