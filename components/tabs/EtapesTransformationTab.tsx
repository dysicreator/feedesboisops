  ...
  },
  {
    ...
    autoFillFields: [
        ...
        },
        {
            sourceField: (entity: Recolte | EtapeTransformation, localAllData) => {
                let baseName = '';
                if ('cultureId' in entity && localAllData) { // Recolte
                    const culture = localAllData.culturesData.find(c => c.id === (entity as Recolte).cultureId);
                    baseName = culture?.nomPlante || 'Produit';
                } else { // EtapeTransformation
                    baseName = (entity as EtapeTransformation).matiereSortanteDescription.split(' - Lot:')[0];
                }
                return `${baseName} (Transformé)`;
            },
            targetFormField: 'matiereSortanteDescription'
        },
        {
            sourceField: (entity: Recolte | EtapeTransformation, allData?: AllData) => {
                if(!allData) return '';
                const baseLotId = 'lotNumero' in entity ? entity.lotNumero : entity.lotSortantId;
                const existingTransformsCount = allData.etapesTransformationData.filter(et => et.lotEntrantId === ('lotNumero' in entity ? entity.id : entity.lotSortantId) ).length;
                return `${baseLotId}-T${existingTransformsCount + 1}`;
            },
            targetFormField: 'lotSortantId'
        }
    ]
  },
...
const EtapesTransformationTab: React.FC<EtapesTransformationTabProps> = ({ currentUser }) => {
  ...
  const handleAddItem = useCallback(() => {
    setEditingEtape({
        ...
        dateDebut: formatDateForInput(new Date().toISOString()),
        lotSortantId: '',
        travailleurs: [],
        statut: 'Planifiée',
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    if (!etapeToDelete) return;

    try {
        const batch = writeBatch(db);

        // Re-credit stock if the transformation was completed and stock was deducted
        if (etapeToDelete.statut === 'Terminée') {
            ...
            if (qtyToAdjust > 0) {
                if (etapeToDelete.typeLotEntrant === 'Recolte') {
                    ...
                    const etapeSource = allData.etapesTransformationData.find(et => et.lotSortantId === etapeToDelete.lotEntrantId);
                    if (etapeSource) {
        ...
        const itemToDeleteRef = doc(db, "etapesTransformationData", id);
        ...
        console.error("Error deleting transformation step:", { message: error.message, code: error.code });
  ...
  const handleFormSubmit = async (data: Partial<EtapeTransformation & {quantiteAUtiliser: number}>) => {
        ...
        if (stockAdjustmentSuccessful) {
            if(finalData.typeLotEntrant === 'Recolte') {
                ...
                if (!sourceRecolte) { stockAdjustmentSuccessful = false; errorMessage = "Lot de récolte source introuvable."; }
                    ...
                    if(oldEtapeData?.lotEntrantId === sourceRecolte.id) availableStock += (oldEtapeData.quantiteEntrante || 0); 
    ...
    try {
        ...
        console.error("Error saving transformation step:", { message: error.message, code: error.code });
