...
const getRecolteFormConfig = (): FormFieldConfig<Recolte>[] => [
  {
    ...
    autoFillFields: [
        ...
        {
            sourceField: (culture: Culture, allData?: AllData) => {
                if (!culture || !allData) return '';
                const prefix = culture.nomPlante.substring(0, 3).toUpperCase();
                const year = new Date().getFullYear().toString().slice(-2);
                const harvestsForThisPlant = allData.recoltesData.filter(r => {
                    const c = allData.culturesData.find(cult => cult.id === r.cultureId);
                    return c?.nomPlante === culture.nomPlante;
                });
                const nextId = (harvestsForThisPlant.length + 1).toString().padStart(3, '0');
                return `${prefix}-${year}-${nextId}`;
            },
            targetFormField: 'lotNumero'
        }
    ]
  },
  ...
  { name: 'lotNumero', label: 'Numéro de Lot de la Récolte', type: 'text', required: true, placeholder: 'Ex: LAV-24-001' },
